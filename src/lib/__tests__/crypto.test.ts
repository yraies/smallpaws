...
describe("Crypto Functions", () => {
  ...
  describe("Form Encryption/Decryption Cycle", () => {
    ...
    test("encrypts and decrypts form-like objects", () => {
      ...
      expect(decrypted.categories[0].questions[0]).toHaveProperty(
        "selection",
        "must",
      );
    });
    ...
    test("handles null and undefined values", () => {
      ...
      const decrypted2 = decryptFormData(encrypted2, testPassword) as Record<
        string,
        unknown
      >;
  ...
  describe("End-to-End Form Storage Simulation", () => {
    test("simulates complete form save/load cycle", () => {
      ...
      expect(categories[0].questions[0]).toHaveProperty(
        "question",
