...
- **Form identity model**: Forms should persist the originating template name (frozen) and record the respondent's name instead of having an editable form title. Recently viewed shared forms should be tracked in browser storage with template identity for filtered comparison suggestions. Labels must not imply server-side data exposure. (F-023)
